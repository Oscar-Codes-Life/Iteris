...
Valid incomplete reviews and unverified acceptance criteria enter a recovery step within the same `maxRepairCycles` budget. Recovery can commit code or test-setup fixes and request additional local test commands; the host runs those commands and configured checks on the resulting commit before a fresh independent review. Evidence-only recovery does not require an empty commit. Repeated gaps stop early, and unavailable external prerequisites are reported explicitly. Malformed reports, wrong-commit evidence, cancellation, and timeouts remain incomplete. Standalone review/audit never runs recovery.
