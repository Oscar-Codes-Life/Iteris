...
Valid incomplete reviews and unverified acceptance criteria enter recovery. Recovery receives unverified defect candidates from incomplete investigator passes, can commit code or test-setup fixes, and can request additional local test commands. The host runs those commands and configured checks on the resulting commit before a fresh independent review. Evidence-only recovery does not require an empty commit. Discovered validation commands are saved with the review scope and survive retries and restarts; code repairs still require fresh checks and review on the new commit. Repeated issues stop after their configured attempt limit; an unchanged recovery without new commands stops immediately. Unavailable hosted CI prerequisites are reported explicitly in the PR and do not hold up PR creation when local checks pass and no unresolved defect or missing requirement remains. If an investigator, verifier, or write-capable recovery agent returns a report that fails schema validation, Iteris launches a separate read-only schema recovery agent with the original task, response, and validation error. Two failed correction attempts defer the review; the unattended ticket queue retries that ticket with backoff until it succeeds or the queue is cancelled. A report-format error never launches a write-capable project recovery agent. Wrong-commit evidence, cancellation, and timeouts remain incomplete. Standalone review/audit reports incomplete when schema recovery fails; it does not run the unattended queue.
