...
test('no GitHub Projects: returns open repository issues instead of requiring a Project',async()=>{
 const requests=[];
 ...
 const result=await fetchTodoTickets(config(),undefined,api);
...
test('explicit Projects mode retains actionable no-project error',async()=>{
 const api=client(url=>{assert.equal(url.pathname,'/graphql');return {body:{data:{organization:{projectsV2:{nodes:[]}}}}};});
 await assert.rejects(fetchTodoTickets({...config(),githubSource:'projects'},undefined,api),/No visible GitHub Projects V2/);
});
