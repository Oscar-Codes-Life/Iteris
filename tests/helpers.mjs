...
if (args[0] === 'app-server') {
 ...
 process.stdin.on('end', async () => {
  ...
  if (prompt.startsWith('ITERIS_REVIEW')) {
   ...
   const needsEvidence=(scenario==='progressive-evidence' && evidenceStage>=0) || (['recover-evidence','recover-verifier','recover-unverified','recover-with-blocker','recover-failed-check','external-gap','external-gap-legacy','external-gap-direct','gap-candidate','recovery-extra-findings'].includes(scenario) && !input.checks.some(c=>c.command==='printf recovery-evidence' && c.exitCode===0));
  ...
  if (prompt.startsWith('ITERIS_RECOVER')) {
   ...
   const legacyCI=process.env.REVIEW_SCENARIO==='external-gap-legacy',externalCI=legacyCI||process.env.REVIEW_SCENARIO==='external-gap';
